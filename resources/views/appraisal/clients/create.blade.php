...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-6">
        <nav class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-2">
            ...
            <span class="text-gray-700 dark:text-gray-300">Tambah Klien Baru</span>
        </nav>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Tambah Klien Baru</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">Isi informasi klien untuk menambahkan ke sistem</p>
    </div>
    ...
    {{-- Form --}}
    <form action="{{ route('appraisal.clients.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            ...
            <div class="form-control">
                ...
                <textarea name="address" rows="3" 
                          ...
                          placeholder="Jalan, nomor, RT/RW, kelurahan, kecamatan, kota, provinsi" required>{{ old('address') }}</textarea>
                @error('address')
                <label class="label"><span class="label-text-alt text-error">{{ $message }}</span></label>
                @enderror
            </div>
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('appraisal.clients.index') }}" class="btn btn-ghost">Batal</a>
