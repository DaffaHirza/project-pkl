...
@section('content')
    <div class="max-w-3xl mx-auto">
        ...
        <form action="{{ route('kanban.assets.update', $asset) }}" method="POST"
            ...
            <div class="mb-6">
                ...
                <select name="asset_type" required
                    ...
                    <option value="tanah" {{ old('asset_type', $asset->asset_type) === 'tanah' ? 'selected' : '' }}>Tanah
                    </option>
                    <option value="bangunan" {{ old('asset_type', $asset->asset_type) === 'bangunan' ? 'selected' : '' }}>
                        Bangunan</option>
                    <option value="tanah_bangunan"
                        {{ old('asset_type', $asset->asset_type) === 'tanah_bangunan' ? 'selected' : '' }}>Tanah & Bangunan
                    </option>
                    <option value="kendaraan"
                        {{ old('asset_type', $asset->asset_type) === 'kendaraan' ? 'selected' : '' }}>Kendaraan</option>
                    <option value="mesin" {{ old('asset_type', $asset->asset_type) === 'mesin' ? 'selected' : '' }}>Mesin
                        & Peralatan</option>
                    <option value="bisnis" {{ old('asset_type', $asset->asset_type) === 'bisnis' ? 'selected' : '' }}>
                        Bisnis</option>
                    <option value="personal_property"
                        {{ old('asset_type', $asset->asset_type) === 'personal_property' ? 'selected' : '' }}>Personal
                        Property</option>
                    <option value="lainnya" {{ old('asset_type', $asset->asset_type) === 'lainnya' ? 'selected' : '' }}>
                        Lainnya</option>
                </select>
            </div>
            ...
            <div class="mb-6">
                ...
                <select name="current_stage"
                    ...
                    @foreach (\App\Models\AssetKanban::STAGES as $num => $name)
                        <option value="{{ $num }}"
                            {{ old('current_stage', $asset->current_stage) == $num ? 'selected' : '' }}>
                            {{ $num }}. {{ $name }}</option>
                    @endforeach
                </select>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Untuk pindah stage, disarankan gunakan tombol stage
                    di halaman detail</p>
            </div>
        ...
        @if (auth()->user()->hasAdminAccess())
            <div class="mt-8 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-6">
                ...
                <p class="text-red-600 dark:text-red-500 text-sm mb-4">Tindakan ini tidak dapat dibatalkan. Semua dokumen
                    dan catatan terkait akan ikut terhapus.</p>
                <form action="{{ route('kanban.assets.destroy', $asset) }}" method="POST"
                    onsubmit="return confirm('Anda yakin ingin menghapus asset ini beserta seluruh dokumen dan catatan?')">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
