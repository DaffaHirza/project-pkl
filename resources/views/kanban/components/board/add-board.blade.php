<!-- Create Board Modal -->
<dialog id="createBoardModal" class="modal modal-bottom sm:modal-middle">
    <div class="modal-box w-full max-w-md p-0">
        <!-- Close Button -->
        <button onclick="document.getElementById('createBoardModal').close()" 
        ...
        <div class="px-6 py-6">
            ...
            <div class="mb-6 pb-4 border-b border-gray-200">
                <h3 class="font-bold text-lg text-gray-900">Buat Board Baru</h3>
            </div>
            ...
            <form action="{{ route('kanban.store') }}" method="POST">
                ...
                <div class="mb-5">
                    ...
                    <input type="text" name="name" required 
                           ...
                           placeholder="Contoh: Product Development">
                    @error('name')
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" rows="3"
                              class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition resize-none"
                              placeholder="Jelaskan tujuan dari board ini..."></textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1.5">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex gap-3 pt-4 border-t border-gray-200">
                    <button type="button" onclick="document.getElementById('createBoardModal').close()" 
                            class="flex-1 px-4 py-2.5 text-gray-700 font-medium border border-gray-300 rounded-lg hover:bg-gray-50 transition">
                        Batal
                    </button>
                    <button type="submit" class="flex-1 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg transition flex items-center justify-center gap-2">
