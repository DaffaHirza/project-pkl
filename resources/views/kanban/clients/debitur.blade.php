...
@section('content')
    <div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            ...
            <a href="{{ route('kanban.clients.create.klien') }}"
                class="inline-flex items-center gap-2 px-4 py-2.5 bg-orange-500 hover:bg-orange-600 text-white rounded-lg font-medium text-sm transition">
        ...
        <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4 mb-6">
            <form action="{{ route('kanban.clients.debitur') }}" method="GET" class="flex flex-col sm:flex-row gap-4">
                ...
                @if (request('search') || request('parent_id'))
                    <a href="{{ route('kanban.clients.debitur') }}"
        ...
        <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-10 h-10 rounded-full {{ $client->type === 'debitur' ? 'bg-orange-100 dark:bg-orange-900/30' : 'bg-green-100 dark:bg-green-900/30' }} flex items-center justify-center flex-shrink-0">
                                            <span
                                                class="text-sm font-semibold {{ $client->type === 'debitur' ? 'text-orange-600 dark:text-orange-400' : 'text-green-600 dark:text-green-400' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $client->type === 'debitur' ? 'bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-400' : 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' }}">
                                        {{ $client->type === 'debitur' ? 'Debitur' : 'PT/CV Anak' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if ($client->parent)
                                        <a href="{{ route('kanban.clients.show', $client->parent) }}"
                                            ...
                                            <span
                                                class="text-xs text-gray-400">({{ $client->parent->type === 'bank' ? 'Bank' : 'PT/CV' }})</span>
                                        </a>
                                    @else
            ...
            @js($client->type === 'debitur' ? 'Debitur' : 'PT/CV Anak'),
            {{ $client->assets_count ?? 0 }}
        )"
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-500 dark:text-gray-400">Belum ada debitur atau anak perusahaan
                                        </p>
                                        <a href="{{ route('kanban.clients.create.klien') }}"
                                            class="mt-2 text-orange-600 hover:text-orange-700 dark:text-orange-400 font-medium">+
                                            Tambah Klien</a>
                                    </div>
                                </td>
                            </tr>
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                            ...
                            asset yang terhubung dengan data ini.
                        </p>
    ...
    <script>
        ...
        function openDeleteClientModal(clientName, formId, clientType, assetCount) {
