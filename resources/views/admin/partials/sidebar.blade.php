    ...
    <div class="flex items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5">
        <a href="{{ url('/') }}" class="flex items-center gap-3">
            <span class="text-2xl flex items-center justify-center font-bold text-brand-600 dark:text-brand-400">
                <div
                    class="w-10 h-10 rounded-lg bg-brand-500 flex items-center justify-center text-white font-bold text-lg">
                    K
                </div>
    ...
    <div class="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
        @php
            ...
            $activeMenu = match (true) {
                ...
                str_starts_with($currentRoute, 'kanban.recapitulations') => 'Kanban.Recapitulations',
                str_starts_with($currentRoute, 'notifications') => 'Notifications',
                str_starts_with($currentRoute, 'assistant') => 'AI Assistant',
                str_starts_with($currentRoute, 'admin.users') => 'Akun',
                $currentRoute === 'admin.account' => 'Akun',
                $currentRoute === 'dashboard' => 'Dashboard',
                default => 'Dashboard',
            };
            $openKanban = str_starts_with($activeMenu, 'Kanban');
        @endphp
        <nav class="px-4 py-4 lg:px-6" x-data="{ selected: '{{ $activeMenu }}', openMenus: {{ $openKanban ? "['Kanban']" : '[]' }} }">
            <!-- Menu Group -->
            <div>
                ...
                <ul class="mb-6 flex flex-col gap-1.5">
                    ...
                    <li>
                        <a href="{{ route('dashboard') }}" @click="selected = 'Dashboard'"
                            ...
                            <svg class="transition-colors duration-300 w-5 h-5"
                                :class="selected === 'Dashboard' ? 'text-brand-500 dark:text-brand-400' :
                            ...
                            <li>
                                <a href="{{ route('kanban.dashboard') }}" @click="selected = 'Kanban.Dashboard'"
                                    ...
                                    <span class="w-1.5 h-1.5 rounded-full"
                                        :class="selected === 'Kanban.Dashboard' ? 'bg-brand-500' : 'bg-gray-400'"></span>
                                    Dashboard
                                </a>
                            </li>
                            <li>
                                <a href="{{ route('kanban.assets.index') }}" @click="selected = 'Kanban.Board'"
                                    ...
                                    :class="selected === 'Kanban.Board' ?
                                        ...
                                        'text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-200'">
                                    <span class="w-1.5 h-1.5 rounded-full"
                                        :class="selected === 'Kanban.Board' ? 'bg-brand-500' : 'bg-gray-400'"></span>
                                    Kanban Board
                                </a>
                            </li>
                            <li>
                                <a href="{{ route('kanban.clients.index') }}" @click="selected = 'Kanban.Clients'"
                                    ...
                                    :class="selected === 'Kanban.Clients' ?
                                        ...
                                        'text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-200'">
                                    <span class="w-1.5 h-1.5 rounded-full"
                                        :class="selected === 'Kanban.Clients' ? 'bg-brand-500' : 'bg-gray-400'"></span>
                                    Klien
                                </a>
                            </li>
                            <li>
                                <a href="{{ route('kanban.activity-log') }}" @click="selected = 'Kanban.ActivityLog'"
                                    ...
                                    :class="selected === 'Kanban.ActivityLog' ?
                                        ...
                                        'text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-200'">
                                    <span class="w-1.5 h-1.5 rounded-full"
                                        :class="selected === 'Kanban.ActivityLog' ? 'bg-brand-500' : 'bg-gray-400'"></span>
                                    Log Aktivitas
                                </a>
                            </li>
                            <li>
                                <a href="{{ route('kanban.recapitulations.index') }}"
                                    @click="selected = 'Kanban.Recapitulations'"
                                    class="group flex items-center gap-2.5 rounded-lg px-4 py-2 font-medium text-sm duration-300 ease-in-out"
                                    :class="selected === 'Kanban.Recapitulations' ?
                                        'text-brand-500 dark:text-brand-400' :
                                        'text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-200'">
                                    <span class="w-1.5 h-1.5 rounded-full"
                                        :class="selected === 'Kanban.Recapitulations' ? 'bg-brand-500' : 'bg-gray-400'"></span>
                                    Rekapitulasi
                                </a>
                            </li>
                        </ul>
                    ...
                    <!-- AI Assistant Menu -->
                    <li>
                        <a href="{{ route('assistant.index') }}" @click="selected = 'AI Assistant'"
                            class="group relative flex items-center gap-2.5 rounded-lg px-4 py-2 font-medium text-sm duration-300 ease-in-out"
                            :class="selected === 'AI Assistant' ?
                                'bg-brand-50 text-brand-500 dark:bg-brand-500/[0.12] dark:text-brand-400' :
                                'text-gray-700 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-300 dark:hover:bg-white/5 dark:hover:text-gray-300'">
                            <svg class="w-5 h-5 transition-colors duration-300"
                                :class="selected === 'AI Assistant' ? 'text-brand-500 dark:text-brand-400' :
                                    'text-gray-500 group-hover:text-gray-700 dark:text-gray-400 dark:group-hover:text-gray-300'"
                    ...
                    <!-- Akun Menu -->
                    <li>
                        <a href="{{ route('admin.users.index') }}" @click="selected = 'Akun'"
                            class="group relative flex items-center gap-2.5 rounded-lg px-4 py-2 font-medium text-sm duration-300 ease-in-out"
                            :class="selected === 'Akun' ?
                                'bg-brand-50 text-brand-500 dark:bg-brand-500/[0.12] dark:text-brand-400' :
                                'text-gray-700 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-300 dark:hover:bg-white/5 dark:hover:text-gray-300'">
                            <svg class="w-5 h-5 transition-colors duration-300"
                                :class="selected === 'Akun' ? 'text-brand-500 dark:text-brand-400' :
                                    'text-gray-500 group-hover:text-gray-700 dark:text-gray-400 dark:group-hover:text-gray-300'"
